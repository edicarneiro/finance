...
| 13 | Frontend Web (MVP) — decomposta em 13.1 a 13.9 (ver ADR-0025) | Interface para todas as funcionalidades acima | Fases 1 a 12 | ✅ Concluída |
...
| 13.4 | Orçamentos e Metas | `BudgetController`, `GoalController` | Fase 13.3 | ✅ Concluída |
| 13.5 | Dashboard e Pulse Score | `DashboardController` | Fase 13.4 | ✅ Concluída |
| 13.6 | Relatórios (incl. exportação CSV) | `ReportController` | Fase 13.3 | ✅ Concluída |
| 13.7 | Notificações e preferências | `NotificationController` | Fase 13.1 | ✅ Concluída |
| 13.8 | Privacidade/LGPD (exportação, consentimento, exclusão de conta) | `PrivacyController`, `UserController` | Fase 13.1 | ✅ Concluída |
| 13.9 | Backoffice (suporte, suspensão/reativação, audit log) | `BackofficeController` | Fase 13.1 | ✅ Concluída |
...
**Nota sobre a Fase 13.4 (decisão do CTO)**: também sem ADR novo, mesmo motivo de 13.2/13.3. O seletor "vincular a conta ou categoria" de metas e a exibição condicional dos campos de período customizado de orçamento são affordances de UI que evitam erros previsíveis do backend (`InvalidGoalAssociationException`, `InvalidBudgetPeriodException`) sem duplicar essas regras — o backend continua sendo a única fonte de verdade, confirmado por verificação manual contra `backend-java` real. A revisão de QA corrigiu uma duplicação verbatim de uma função utilitária (`toThresholdList`) entre `BudgetsPage` e `GoalsPage`, extraída para um módulo compartilhado.

**Nota sobre a Fase 13.5 (decisão do CTO)**: também sem ADR novo, mesmo motivo de 13.2–13.4. `DashboardPage` substitui o placeholder `HomePage` na rota `/` — a revisão de QA encontrou e corrigiu dois achados reais: (1) a troca de dono da rota `/` quebrava testes de fases anteriores (`LoginPage`/`RegisterPage`/logout) que passam por ali incidentalmente após autenticação, corrigido com handlers MSW padrão sempre ativos para `/dashboard`; (2) um seletor de período posicionado dentro de "Fluxo de caixa" também recalculava silenciosamente "Gastos por categoria" (confirmado contra `GetDashboardUseCase` real, que usa a mesma janela de dias para os dois), corrigido movendo o controle para fora das duas seções com rótulo explícito do escopo compartilhado. O Pulse Score é exibido com sua ressalva de fórmula provisória (RN-006), sem biblioteca de gráficos (decisão de escopo, não dívida técnica).

**Nota sobre a Fase 13.6 (decisão do CTO)**: também sem ADR novo, mesmo motivo de 13.2–13.5. `httpClient.downloadFile` (novo) resolve o download de arquivo autenticado (CSV) sem um `<a href>` simples, que não enviaria o header `Authorization` — mesmo tratamento de erro/sessão expirada das demais chamadas. A revisão de QA encontrou e corrigiu dois achados reais: (1) as duas seções de leitura mostravam uma mensagem de erro genérica em vez da real do backend, inconsistente com o padrão já estabelecido em todas as outras páginas; (2) ao escrever o teste de regressão do achado anterior, descobriu-se que `useQuery` tenta de novo 3x com backoff antes de assentar em erro (diferente de `useMutation`, que não tenta de novo) — um comportamento de **produção real**, não só de teste, que atrasaria a exibição de qualquer erro determinístico em qualquer tela do projeto baseada em `useQuery`. Corrigido com `retry: false` global em `queryClient.ts`, beneficiando todas as fases anteriores retroativamente.

**Nota sobre a Fase 13.7 (decisão do CTO)**: também sem ADR novo, mesmo motivo de 13.2–13.6. `NotificationsPage` consome `NotificationController` em três seções independentes: matriz de preferências (3 tipos de alerta × 2 canais), verificação sob demanda e lista com filtro de não lidas. RF-043 (lembrete de transação recorrente) permanece sem tela, pelo mesmo motivo já registrado na Fase 10 (ADR-0022): depende de RF-016 (transações recorrentes, Fase 4.2, ainda não construída). A revisão de QA encontrou e corrigiu um achado real: o estado local dos toggles de preferência era resincronizado a partir do servidor a cada refetch de fundo do TanStack Query (ex.: o navegador reganhando foco, `refetchOnWindowFocus: true` por padrão), descartando silenciosamente alterações não salvas do usuário — corrigido com uma guarda de sincronização única (`useRef`), documentada em `frontend/README.md` como padrão geral para qualquer tela futura que sincronize estado local editável a partir de uma `useQuery`.

**Nota sobre a Fase 13.8 (decisão do CTO)**: também sem ADR novo, mesmo motivo de 13.2–13.7. `PrivacyPage` consome `PrivacyController`/`UserController` em três seções: exportação de dados (download de JSON montado no cliente, já que o endpoint real não envia `Content-Disposition` de anexo, ao contrário das exportações CSV da Fase 13.6), consentimento (registro + histórico), e exclusão de conta com reautenticação por senha — o aviso de que dados financeiros não são apagados nem anonimizados reflete literalmente a decisão já registrada no ADR-0023. A revisão de QA encontrou e corrigiu um achado real de severidade alta: `httpClient.apiRequest` tratava qualquer `401` de chamada autenticada como sessão expirada, mas `DELETE /users/me` também retorna `401` quando a senha de reautenticação está errada (erro de validação de negócio, não sessão inválida) — uma senha de confirmação digitada errada deslogava o usuário silenciosamente no meio do fluxo de exclusão. Corrigido com uma nova opção `treatUnauthorizedAsSessionExpired` no módulo de rede compartilhado, preservando o comportamento padrão em todas as demais chamadas. A exclusão bem-sucedida foi deliberadamente não exercitada contra o backend real (anonimizaria o usuário de dev seedado, do qual a Fase 13.9 ainda depende), permanecendo coberta pela suíte automatizada.

**Nota sobre a Fase 13.9 (decisão do CTO)**: também sem ADR novo — a decisão de RBAC pendente desde a Fase 13.1 (como o frontend deveria tratar a ausência de claim de papel no JWT, ver ADR-0024) foi resolvida aplicando o mesmo ADR-0024 já existente à camada de apresentação, não uma decisão arquitetural nova: o link "Backoffice" no `AppShell` é sempre visível para qualquer usuário autenticado, e o controle de acesso real é imposto pelo backend a cada chamada (`OperatorAuthorization.requireSupportOperator`), com a página exibindo a mensagem de erro 403 real para quem não tiver a permissão. `BackofficePage` reaproveita o tipo de resposta já usado pela exportação de dados da Fase 13.8 (`GetUserForSupportUseCase` chama `ExportUserDataUseCase` internamente — mesmo formato de dado). A revisão de QA encontrou e corrigiu uma divergência de fidelidade do mock: o backend real registra uma entrada `VIEWED_USER_DATA` no log de auditoria em toda busca de suporte (RF-048), não só nas ações de suspender/reativar — descoberto apenas na verificação manual contra `backend-java` real (promovendo o usuário de dev a `SUPPORT_OPERATOR` diretamente via SQL, já que não há endpoint de autopromoção), não pela suíte automatizada isolada. **Esta fase encerra a decomposição completa da Fase 13 (13.1–13.9, ver ADR-0025) e, com ela, a Parte 1 (MVP) do produto FinancePulse Engine, backend e frontend.**
...
| 13.4 (Frontend) | ✅ [docs/cto/fase-13-4-frontend-aprovacao.md](docs/cto/fase-13-4-frontend-aprovacao.md) | *aguardando* | nenhum novo (ver ADR-0025) | [docs/qa/fase-13-4-frontend-review.md](docs/qa/fase-13-4-frontend-review.md) |
| 13.5 (Frontend) | ✅ [docs/cto/fase-13-5-frontend-aprovacao.md](docs/cto/fase-13-5-frontend-aprovacao.md) | *aguardando* | nenhum novo (ver ADR-0025) | [docs/qa/fase-13-5-frontend-review.md](docs/qa/fase-13-5-frontend-review.md) |
| 13.6 (Frontend) | ✅ [docs/cto/fase-13-6-frontend-aprovacao.md](docs/cto/fase-13-6-frontend-aprovacao.md) | *aguardando* | nenhum novo (ver ADR-0025) | [docs/qa/fase-13-6-frontend-review.md](docs/qa/fase-13-6-frontend-review.md) |
| 13.7 (Frontend) | ✅ [docs/cto/fase-13-7-frontend-aprovacao.md](docs/cto/fase-13-7-frontend-aprovacao.md) | *aguardando* | nenhum novo (ver ADR-0025) | [docs/qa/fase-13-7-frontend-review.md](docs/qa/fase-13-7-frontend-review.md) |
| 13.8 (Frontend) | ✅ [docs/cto/fase-13-8-frontend-aprovacao.md](docs/cto/fase-13-8-frontend-aprovacao.md) | *aguardando* | nenhum novo (ver ADR-0025) | [docs/qa/fase-13-8-frontend-review.md](docs/qa/fase-13-8-frontend-review.md) |
| 13.9 (Frontend) | ✅ [docs/cto/fase-13-9-frontend-aprovacao.md](docs/cto/fase-13-9-frontend-aprovacao.md) | *aguardando* | nenhum novo (ver ADR-0025/ADR-0024) | [docs/qa/fase-13-9-frontend-review.md](docs/qa/fase-13-9-frontend-review.md) |
